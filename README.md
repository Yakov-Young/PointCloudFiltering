...
| Компонент | Версия |
|-----------|--------|
| ОС | Windows 10 22H2 64-bit |
| Python | 3.12 |
| CPU | 6 логических ядер |
| Пакеты | см. `./requirements.txt` |

Примечание: модули `./batch_analysis.py` и `./report.py` используют в своей реализации `n_jobs=6`, т. е. 6 логических ядер.
